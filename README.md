...
## Overview of Project
Jeremy is approached by upper management about a special project a few weeks after starting his new job. The MechaCar, AutosRUs' newest prototype, is experiencing production issues that are impeding the manufacturing team's progress. Upper management at AutosRUs has asked Jeremy and the data analytics team to review production data for insights that can help the manufacturing team.

You will assist Jeremy and the data analytics team with the following tasks in this challenge:

* Conduct multiple linear regression analysis to determine which variables in the dataset predict MechaCar prototype mpg.
* Gather summary statistics on the suspension coils' pounds per square inch (PSI) from manufacturing lots.
* Use t-tests to see if the manufacturing lots differ statistically from the mean population.
* Create a statistical study to compare MechaCar vehicle performance to that of other manufacturers' vehicles. You will write a summary interpretation of the findings for each statistical analysis.
